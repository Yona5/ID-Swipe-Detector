...
public class AttendanceController{
    ...
    public void run() {
        ...
        do {
            ...
            switch (choice) {
                ...
                case 'Q': 
                    InputHelper inputHelper = new InputHelper();
                    char ans = inputHelper.readCharacter("Do you want to save the file? Y/N");
                    if(Character.toUpperCase(ans) == 'Y'){
                        String export = inputHelper.readString("Type file name");
                        repository.store(export);
                    }
                    else{
                        finished = true;
                    }     
    ...
    private void listSwipesByCardIdOrderedByDateTime() {        
        ...
        System.out.format("\033[31m%s\033[0m%n", "=================");
        Collections.sort(repository.getItems());
        InputHelper inputHelper = new InputHelper();
        String ans = inputHelper.readString("Enter card ID");
        ListIterator iterator = repository.getItems().listIterator(repository.getItems().size());
        while(iterator.hasPrevious()){
            Swipe swipe = (Swipe)iterator.previous();
            if(swipe.getCardId().toUpperCase().equals(ans.toUpperCase())){
    ...
    private void listSwipeStatistics() {
        ...
        int counter = 0;
        InputHelper inputHelper = new InputHelper();
        String ans = inputHelper.readString("Enter room ");
        for(Swipe item:repository.getItems()){
            if(item.getRoom().toUpperCase().equals(ans.toUpperCase())){
                counter++;
                room.add(item.formatSwipeDateTime(item.getSwipeDateTime()));
            }
        }
        System.out.println("Number of swipes: " + counter);
        System.out.println("Last swipe time: " + room.get(room.size() - 1));
    }
}
